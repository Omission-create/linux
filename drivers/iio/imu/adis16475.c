...
#define ADIS16475_OVERWRITE_OLDEST	FIELD_PREP(ADIS16475_OVERFLOW_MASK, 0)
...
static int adis16475_buffer_postenable(struct iio_dev *indio_dev)
{
	...
	ret = __adis_update_bits(adis, ADIS16475_REG_FIFO_CTRL, ADIS16475_WM_LVL_MASK, val);
	if (ret)
		return ret;

	/* Update overflow behavior to always overwrite the oldest sample. */
	val = ADIS16475_OVERWRITE_OLDEST;
	ret = __adis_update_bits(adis, ADIS16475_REG_FIFO_CTRL, ADIS16475_OVERFLOW_MASK, val);
	if (ret)
		return ret;
	...
	ret = __adis_update_bits(adis, ADIS16475_REG_FIFO_CTRL, ADIS16475_WM_EN_MASK, val);
	if (ret)
		return ret;
...
static int adis16475_buffer_postdisable(struct iio_dev *indio_dev)
{
	...
	/* Disable watermark interrupt pin. */
	val = ADIS16475_WM_EN(0);
	ret = __adis_update_bits(adis, ADIS16475_REG_FIFO_CTRL, ADIS16475_WM_EN_MASK, val);
	if (ret)
		return ret;
...
static int adis16475_config_irq_pin(struct adis16475 *st)
{
	...
	if (st->info->flags & ADIS16475_HAS_FIFO) {
		...
		ret = __adis_update_bits(&st->adis, ADIS16475_REG_FIFO_CTRL,
